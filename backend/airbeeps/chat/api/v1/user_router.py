...
from airbeeps.assistants.models import Conversation, Message, MessageTypeEnum
...
async def submit_message_feedback(
    ...
    """
    Store thumbs up/down feedback for a specific assistant message.

    Enforces:
    - Message must exist and belong to the current user (via its conversation)
    - Message must be an ASSISTANT message
    - One feedback per (message, user); subsequent submits update the existing row
    """
    # Load message + conversation for ownership check
    stmt = (
        select(Message)
...
async def chat_with_assistant(
    ...
    from airbeeps.assistants.models import AssistantStatusEnum
    ...
    conversation = await service.get_conversation(
        chat_data.conversation_id, current_user.id
    )
    if not conversation:
        raise HTTPException(status_code=404, detail="Conversation not found")
