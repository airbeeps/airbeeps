#!/usr/bin/env python3
"""Create a new app module skeleton inside airbeeps/.
...
MODELS_TEMPLATE = """from airbeeps.model import Base  # SQLAlchemy declarative Base
from sqlalchemy.orm import Mapped, mapped_column
"""

ROOT_ROUTER_IMPORT_HINT = """
# To enable this module's routes, add the following line in airbeeps/main.py:
...
def create_module(name: str, force: bool = False) -> None:
    ...
    if target.exists() and not force:
        print(
            f"[ERROR] {target} already exists. Use --force to overwrite (will not delete existing files).",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a new application module skeleton"
    )
    parser.add_argument("name", help="New module (package) name under airbeeps/")
    parser.add_argument(
    ...
    if not SRC.exists():
        print("[ERROR] airbeeps directory not found", file=sys.stderr)
        sys.exit(2)
