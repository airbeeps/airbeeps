...
export function useModelViewAPI<T extends Record<string, any>>(
  ...
  const getList = async (params: ModelViewQueryParams = {}): Promise<PaginatedResponse<T>> => {
    try {
      ...
      Object.entries(params).forEach(([key, value]) => {
        if (
          !["page", "size", "search", "sort_by", "sort_desc"].includes(key) &&
          value !== undefined &&
          value !== null &&
          value !== ""
        ) {
      ...
      const url = query.toString() ? `${listEndpoint}?${query.toString()}` : listEndpoint;
  ...
  const bulkOperation = async (
    operation: ModelViewBulkOperation
  ): Promise<ModelViewApiResponse> => {
    try {
      // Simulate bulk operation success
      return {
        success: true,
        message: t("common.bulkOperationSuccess", {
