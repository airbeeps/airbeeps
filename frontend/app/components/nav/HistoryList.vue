...
import { MoreHorizontal, Trash2, ChevronDown, Pencil } from "lucide-vue-next";
...
const { removeConversation, fetchConversations, loadMore, updateConversationTitle } =
  conversationsStore;

const getDisplayTitle = (conversation: any) => {
  const title = conversation?.title?.trim();
  return title && title.length > 0
    ? title
    : t("pages.conversations.untitledConversation", "Untitled Conversation");
};
    ...
    <SidebarMenu>
        ...
        <SidebarMenuButton as-child :is-active="route.path === `/chat/${item.id}`">
          ...
          <div v-else class="flex w-full items-center">
            <input
              ...
              :placeholder="getDisplayTitle(item)"
              autofocus
            />
          </div>
        </SidebarMenuButton>
      ...
      <SidebarMenuItem v-if="hasConversations && totalCount > conversations.length">
        ...
        >
          ...
          <span v-else-if="hasNextPage"
            >{{ $t("nav.loadMore") }} ({{ conversations.length }}/{{ totalCount }})</span
          >
