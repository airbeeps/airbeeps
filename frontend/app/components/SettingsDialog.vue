...
const formSchema = toTypedSchema(
  z.object({
    nickname: z.string().max(50, t("profile.nicknameMaxLength")).optional().nullable(),
...
async function onPasswordSubmit(values: any) {
  ...
  try {
    ...
    const fetchError = error as { data?: { detail?: string | { code?: string; reason?: string } } };
    if (fetchError?.data?.detail === "CHANGE_PASSWORD_INCORRECT_OLD_PASSWORD") {
      toast.error(t("profile.security.incorrectOldPassword"));
    } else if (
      typeof fetchError?.data?.detail === "object" &&
      fetchError.data.detail?.code === "CHANGE_PASSWORD_INVALID_PASSWORD"
    ) {
      toast.error(t("profile.security.invalidPassword") + ": " + fetchError.data.detail.reason);
    } else {
  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="max-h-[85vh] p-0 sm:max-w-[700px]">
      ...
      <div class="flex min-h-[400px]">
        <!-- Left tab navigation -->
        <div class="bg-muted/10 w-48 border-r p-4">
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-if="activeTab === 'security'">
            ...
            >
              <form
                @submit="
                  handleSubmit($event, async (vals: any) => {
                    await onPasswordSubmit(vals);
                    resetForm();
                  })
                "
                class="space-y-4"
