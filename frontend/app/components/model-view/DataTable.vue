...
import { ref, computed, watch, onMounted, isVNode } from "vue";
...
const table = useVueTable({
  ...
  meta: {
    triggerRowAction: (action: ActionConfig, row: T) => handleRowAction(action, row),
  },
  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- Create button -->
        <Button v-if="canCreate" @click="$emit('create')" data-testid="create-btn">
          ...
          <TableRow>
            ...
            >
              ...
              >
                ...
                <component :is="getSortIcon(column.column.getIsSorted())" class="h-4 w-4" />
              </div>
              <span v-else>{{ column.column.columnDef.header }}</span>
            </TableHead>

            <!-- Actions column -->
            <TableHead v-if="rowActions.length > 0" class="w-20">{{
              t("components.dataTable.actions")
            }}</TableHead>
          </TableRow>
        </TableHeader>
            ...
            <TableCell v-if="rowActions.length > 0">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  >
                    <component
                      v-if="action.icon && !isVNode(action.icon)"
                      :is="action.icon"
                      class="mr-2 h-4 w-4"
                    />
                    <component
                      v-else-if="action.icon && isVNode(action.icon)"
                      :is="(action.icon as any).type"
                      v-bind="(action.icon as any).props"
                      class="mr-2 h-4 w-4"
                    />
                    {{ action.label }}
                  </DropdownMenuItem>
          ...
          <TableRow v-if="table.getRowModel().rows.length === 0">
            <TableCell :colspan="totalColumns" class="h-24 text-center">
              <div class="flex flex-col items-center gap-2">
                <DatabaseIcon class="text-muted-foreground h-8 w-8" />
                <span class="text-muted-foreground">{{
                  emptyMessage || t("components.dataTable.noData")
                }}</span>
              </div>
            </TableCell>
          </TableRow>
    ...
    <div class="flex items-center justify-between">
      <div class="text-muted-foreground text-sm">
        ...
        {{ pagination.pageIndex * pagination.pageSize + 1 }} {{ t("components.dataTable.to") }}
        {{ Math.min((pagination.pageIndex + 1) * pagination.pageSize, totalCount) }}
        {{ t("components.dataTable.of") }} {{ totalCount }} {{ t("components.dataTable.items") }}
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem v-for="size in [10, 20, 50, 100]" :key="size" :value="size.toString()">
              {{ size }}
            </SelectItem>
          </SelectContent>
        </Select>
