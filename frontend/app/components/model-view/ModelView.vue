...
import { Download, Upload, Trash2, Edit, Eye, Copy } from "lucide-vue-next";
...
// Hide export/import by default; only show when explicitly enabled
const showExport = computed(() => props.config.showExport === true);
const showImport = computed(() => props.config.showImport === true);
const emptyMessage = computed(
  () => props.config.emptyMessage || t("components.modelView.emptyMessage")
);
...
const deleteMessage = computed(() => {
  ...
  return t("components.modelView.deleteConfirmMultiple", {
    count: itemsToDelete.value.length,
    title: title.value,
  });
});
...
const handleFormChange = (data: Record<string, any>) => {
  if (!props.config.onFormChange) return;
  const formApi = formRef.value as any;
  props.config.onFormChange({
    formData: data,
    mode: formMode.value,
...
const editFromDetail = () => {
  if (selectedItem.value) {
    // Set formMode first, then close detail dialog without clearing selectedItem
    // (we need selectedItem for the form's initial-data prop)
    formMode.value = "edit";
    showDetailDialog.value = false;
    // Open form dialog - selectedItem is already set, so form will have data
    showFormDialog.value = true;
  }
};
...
watch(
  () => [props.config.apiEndpoint, props.config.apiEndpoints],
  (newVal, oldVal) => {
    if (
      oldVal &&
      (newVal[0] !== oldVal[0] || JSON.stringify(newVal[1]) !== JSON.stringify(oldVal[1]))
    ) {
  ...
  <div class="model-view space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <Button v-if="showExport" variant="outline" @click="handleExport" :disabled="loading">
          <Download class="mr-2 h-4 w-4" />
          {{ t("components.modelView.export") }}
        </Button>

        <Button v-if="showImport" variant="outline" @click="handleImport" :disabled="loading">
      ...
      <DialogContent class="max-h-[90vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {{
              formMode === "create"
                ? t("components.modelView.createTitle", { title })
                : t("components.modelView.editTitle", { title })
            }}
          </DialogTitle>
          <DialogDescription>
            {{
              formDescription ||
              (formMode === "create"
                ? t("components.modelView.createDescription", { title })
                : t("components.modelView.editDescription", { title }))
            }}
          </DialogDescription>
        </DialogHeader>
        ...
        <DynamicForm
          ...
          :submit-text="
            formMode === 'create'
              ? t('components.modelView.create')
              : t('components.modelView.save')
          "
    ...
    <Dialog v-model:open="showDetailDialog">
      <DialogContent class="max-h-[90vh] max-w-4xl overflow-y-auto">
        ...
        <div v-if="selectedItem" class="space-y-4">
          <div v-for="field in detailFields" :key="field.name" class="grid grid-cols-3 gap-4">
            ...
            <div class="col-span-2">
              ...
              <template v-else-if="field.type === 'json'">
                <pre class="bg-muted/40 rounded p-3 font-mono text-xs whitespace-pre-wrap">{{
