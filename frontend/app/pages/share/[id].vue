...
const assistantName = computed(
  () => localizedAssistant.value?.name ?? shareData.value?.assistant_name ?? ""
);
const assistantDescription = computed(
  () => localizedAssistant.value?.description ?? shareData.value?.assistant_description ?? ""
);
const assistantId = computed(
  () => localizedAssistant.value?.id ?? shareData.value?.assistant_id ?? null
);
    ...
    <div class="mx-auto max-w-3xl space-y-8 px-4 py-10">
      <header class="space-y-4">
        ...
        <div
          ...
          class="border-border bg-card/40 flex flex-col gap-4 rounded-xl border p-4 md:flex-row md:items-center md:justify-between"
        >
      ...
      <section v-if="pending" class="text-muted-foreground flex flex-col items-center gap-2 py-10">
