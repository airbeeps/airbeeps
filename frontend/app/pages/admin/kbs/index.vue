...
const { data: models } = useAPI<Model[]>("/v1/admin/all-models?capabilities=embedding", {
  server: false,
});
...
const knowledgeBaseConfig = computed(
  (): ModelViewConfig<KnowledgeBase> => ({
    ...
    detailFields: [
      ...
      },
      {
        ...
        type: "text",
        render: (val: string) =>
          val ? modelMap.value[val]?.display_name || val : t("admin.pages.knowledgeBases.notSet"),
      },
      {
        name: "chunk_size",
        ...
        render: (val: number) =>
          val
            ? t("admin.pages.knowledgeBases.charactersCount", { count: val })
            : t("admin.pages.knowledgeBases.useDefault"),
      },
      {
        name: "chunk_overlap",
...
<template>
  <div class="space-y-6">
    ...
    <ModelView :config="knowledgeBaseConfig" @row-action="handleKnowledgeBaseAction" />
  </div>
</template>
