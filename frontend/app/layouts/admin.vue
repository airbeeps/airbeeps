...
const breadcrumbs = computed(() => {
  ...
  return paths
    .map((path) => {
      ...
      const name =
        routeRecord.meta?.breadcrumb || routeRecord.meta?.name || routeRecord.name || path;

      // Handle string translation keys - check if translation exists
      if (typeof name === "string") {
        // Try to translate, fallback to original if no translation exists
        const translated = t(name, name);
        return {
          name: translated === name && !name.includes(".") ? name : translated,
          path,
        };
      }
...
<template>
  <SidebarProvider :default-open>
    ...
    <SidebarInset class="min-h-screen min-w-0">
      ...
      <div class="flex w-full flex-1 flex-col gap-4 overflow-x-hidden p-4">
        <slot />
      </div>
    </SidebarInset>
  </SidebarProvider>
